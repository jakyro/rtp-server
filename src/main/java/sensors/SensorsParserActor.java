...
public class SensorsParserActor extends MyActor {
    public static final String host_name = "sensors_parser";
    public static final String publish_name = "sensors_publisher";
    int workersCount = 0;
    int currentWorker = 0;
    ...
    public static void main(String[] args) {
        new SensorsParserActor(args[0], Long.parseLong(args[1]));
    }

    SensorsParserActor(String name, long pid) {
        super(name, pid);
    }
    ...
    private void deliverPacketToParser(Packet packet) {
        ...
        if (currentWorker == 0) {
            ...
            if (!isActorAlive(workerName)) {
                spawn(workerName, SensorsParserActor.class);
            }
    ...
    private void parseInternal(Packet packet) {
        ...
        try {
            MySensorReceive e = mapper.readValue(packet.getData(), MySensorReceive.class);
            sendToPublisher(formatString(e));
        } catch (IOException e) {
        }
    }

    private String formatString(MySensorReceive e) {
        try {
            ObjectMapper mapper = new ObjectMapper();
